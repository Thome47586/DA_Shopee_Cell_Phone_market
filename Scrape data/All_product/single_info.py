...
def get_single_info_product(element, json_info):
    ...
    try:
        # if 'name' in json_info.keys() and 'aggregateRating' in json_info.keys():
        #     if name.text == json_info['name']:
    ...
    # try:
    #   d['rating'], d['rating_count'] = Full_pipe_crawl.get_info_css(name.text)
    #   #d['rating'] = Full_pipe_crawl.get_info_css(name.text)
    # except NoSuchElementException:
    #   pass
